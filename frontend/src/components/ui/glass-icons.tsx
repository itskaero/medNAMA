...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div className={`grid gap-[4em] grid-cols-3 sm:grid-cols-6 mx-auto py-[2.5em] overflow-visible ${className || ''}`}>
      {items.map((item, index) => (
        <button
          ...
          className={`relative bg-transparent outline-none border-none cursor-pointer w-[4.2em] h-[4.2em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group mx-auto ${
            item.customClass || ''
          }`}
        >
          <span
          ...
          <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-[0.72rem] font-medium tracking-wide text-neutral-400 opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(15%)] font-sans">
            {item.label}
          </span>
        </button>
      ))}
